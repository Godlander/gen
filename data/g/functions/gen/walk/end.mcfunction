...
execute store result score walkers path if entity @e[tag=spawn]
execute if score walkers path < paths gen run scoreboard players set steps gen -1
#reset
...
execute if score steps gen matches -1 if score fails gen matches ..5 run schedule function g:gen/start 1t
